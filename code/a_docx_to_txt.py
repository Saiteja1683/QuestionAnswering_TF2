...
print(stop_words)
csv_data = pd.read_csv('./data/ValidationSet.csv')
...
main_dirc = './data/Validation_Data/'  # training doc files
output_txt_dirc = './data/validation_txt_data/'
...
for i in files:
	...
	for n in range(len(doc.paragraphs)):
		...
		if len(paragraphs)!=0:
			...
			lis_nnp = [i[0] for i in tagged if i[-1]=="NNP" or i[-1]=="NP" or i[-1]=="CD" or i[-1]=="RB" or i[-1]=="NNS" or i[-1]=="JJ" or i[-1]=="NN" or i[-1]=="VBP"]
